...
export function DocsSearch({ large = false }: { large?: boolean }) {
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      <Search className={(large ? "left-4 h-4 w-4" : "left-3 h-4 w-4") + " pointer-events-none absolute top-1/2 -translate-y-1/2 text-zinc-400"} />
      <input
        type="search"
        value={query}
        onChange={(event) => {
        ...
        className={
          ...
          " w-full rounded-xl border border-zinc-200 bg-white text-zinc-900 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
        }
      />
      {open && query.trim() && (
        <div className="absolute left-0 right-0 top-full z-30 mt-2 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
          {results.length > 0 ? (
            <div className="max-h-96 overflow-y-auto p-2">
              {results.map((item) => (
                <button
                  ...
                  className="block w-full rounded-xl px-3 py-2 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800"
                >
                  <span className="flex items-center justify-between gap-3">
                    <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{item.title}</span>
                    <span className="rounded-lg bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
                      {item.section}
                    </span>
                  </span>
                  <span className="mt-1 block text-xs leading-5 text-zinc-500 dark:text-zinc-500">{item.description}</span>
                </button>
              ))}
            </div>
          ) : (
