...
type Language = { label: string; icon: string; language: string; version: string; defaultCode: string; color: string };
...
const languages: Language[] = [
  { label: "Python", icon: "🐍", language: "python", version: "3.12.0", color: "bg-blue-500 hover:bg-blue-600", defaultCode: `# Python 3.12
...
for name in names:
    print(greet(name))` },
  { label: "Ruby", icon: "💎", language: "ruby", version: "3.0.1", color: "bg-red-500 hover:bg-red-600", defaultCode: `# Ruby 3.0
def greet(name)
  "Hello, #{name}!"
end
...
["World", "Ruby", "DevTools"].each do |name|
  ...
  { label: "PHP", icon: "🐘", language: "php", version: "8.2.3", color: "bg-violet-500 hover:bg-violet-600", defaultCode: `<?php
...
foreach ($names as $name) {
  ...
  { label: "Perl", icon: "🐪", language: "perl", version: "5.36.0", color: "bg-yellow-600 hover:bg-yellow-700", defaultCode: `use strict;
use warnings;
...
foreach my $name (@names) {
  ...
  { label: "Bash", icon: "$", language: "bash", version: "5.2.0", color: "bg-zinc-700 hover:bg-zinc-800", defaultCode: `#!/bin/bash
greet() {
    echo "Hello, $1!"
}
...
for name in "World" "Bash" "DevTools"; do
  ...
  { label: "Lua", icon: "🌙", language: "lua", version: "5.4.4", color: "bg-indigo-500 hover:bg-indigo-600", defaultCode: `-- Lua 5.4
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {languages.map((language, index) => (
            <button key={language.label} type="button" onClick={() => setActiveIndex(index)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold ${index === activeIndex ? `${language.color} text-white` : "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"}`}>
              <span className="mr-2">{language.icon}</span>{language.label}
            </button>
          ))}
        </div>
        <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <textarea value={code} onChange={(event) => setCode(event.target.value)} onKeyDown={handleKeyDown} spellCheck={false} className="min-h-[360px] w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 font-mono text-sm text-zinc-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100" />
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            <button type="button" onClick={runCode} disabled={running} className={`inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-white ${active.color} disabled:cursor-not-allowed disabled:opacity-60`}>{running && <RefreshCw className="h-4 w-4 animate-spin" />}{running ? "Running..." : `▶ Run ${active.label}`}</button>
            <span className="text-xs text-zinc-400 dark:text-zinc-600">Ctrl+Enter to run</span>
          </div>
        </section>
        <section className="rounded-2xl border border-zinc-800 bg-zinc-950 shadow-sm">
          ...
          <pre className="min-h-[220px] max-h-[400px] overflow-auto whitespace-pre-wrap p-4 font-mono text-sm">
            ...
            {hasRun && outputTab === "errors" && errors && errors.split("\n").map((line, index) => <span key={`e-${index}`} className="block text-red-400">✗ {line}</span>)}
