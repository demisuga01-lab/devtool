...
const dialects: Array<[SqlLanguage, string]> = [
...
export default function SqlFormatterPage() {
  ...
  const run = (minify = false) => {
    try {
      setError("");
      setOutput(minify ? input.replace(/\s+/g, " ").trim() : format(input, { language: dialect, tabWidth: 2 }));
    } catch (e) {
      setOutput("");
  ...
  return (
    <ToolShell>
      ...
      <div className="space-y-5">
        <div><Label>Dialect</Label><ToolSelect value={dialect} onChange={(e) => setDialect(e.target.value as SqlLanguage)}>{dialects.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</ToolSelect></div>
