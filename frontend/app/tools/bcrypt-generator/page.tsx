...
import { Badge, Button, CopyButton, ResultCard, ToolInput, Label, ToolTextarea } from "@/components/tool-ui";
...
function bcryptDetails(value: string) {
  const match = value.match(/^(\$2[aby]\$)(\d{2})\$([./A-Za-z0-9]{22})([./A-Za-z0-9]{31})$/);
  ...
  return {
    ...
    salt: match[3],
    digest: match[4],
    estimatedWork: `${Math.pow(2, cost).toLocaleString()} key setup rounds`,
  };
}
...
export default function BcryptGeneratorPage() {
  ...
  const hashInfo = bcryptDetails(hash);
  const verifyInfo = bcryptDetails(verifyHash);
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Panel noPadding className="space-y-4 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <Badge variant={cost >= 12 ? "success" : cost >= 10 ? "info" : "warning"}>cost {cost}</Badge>
          </div>
          <div>
          ...
          {hashInfo && (
            <div className="grid gap-3 sm:grid-cols-2">
              <ResultCard label="Version" value={hashInfo.version} />
              <ResultCard label="Cost" value={String(hashInfo.cost)} />
              <ResultCard label="Salt" value={hashInfo.salt} mono copyable />
              <ResultCard label="Work Factor" value={hashInfo.estimatedWork} />
            </div>
          )}
        </Panel>
        <Panel noPadding className="space-y-4 p-4">
          ...
          {verifyInfo && (
            <div className="flex flex-wrap gap-2">
              <Badge variant="info">version {verifyInfo.version}</Badge>
              <Badge variant="info">cost {verifyInfo.cost}</Badge>
              <Badge variant="info">salt extracted</Badge>
              {verifyInfo.cost < 10 && <Badge variant="warning">low cost for new hashes</Badge>}
            </div>
          )}
