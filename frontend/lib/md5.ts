/* Minimal MD5 implementation for client-side hashing. Public-domain style. */
...
function md51(bytes: Uint8Array): number[] {
  ...
  const remainder = bytes.subarray(i - 64);
  let j: number;
  for (j = 0; j < remainder.length; j++) {
    tail[j >> 2] |= remainder[j] << ((j % 4) << 3);
  }
  tail[j >> 2] |= 0x80 << ((j % 4) << 3);
  if (j > 55) {
    md5cycle(state, tail);
...
function md5blk(bytes: Uint8Array, offset: number): number[] {
...
export function md5(input: string): string {
  return hex(md51(new TextEncoder().encode(input)));
}
